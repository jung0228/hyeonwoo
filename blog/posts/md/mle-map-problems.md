...
<p><strong>용어 먼저 — 베르누이 우도란?</strong> 동전 던지기처럼 결과가 두 가지(성공/실패)인 시행에서, $n$번 중 $k$번 성공할 확률은 $L(p) = p^k(1-p)^{n-k}$. 이것이 베르누이 우도(likelihood)다. <span class="kw">MLE는 "어떤 $p$ 값이 관측 결과를 가장 그럴듯하게 만드는가?"를 구하는 것</span>이다.</p>

<p><strong>(1) MLE 계산.</strong> $L(p) = p^7(1-p)^3$. 로그를 취하면 다루기 쉽다: $\ell(p) = 7\log p + 3\log(1-p)$. 미분해서 0으로 놓으면:</p>
<div class="prob-formula">$$\frac{d\ell}{dp} = \frac{7}{p} - \frac{3}{1-p} = 0 \implies 7(1-p) = 3p \implies \hat{p}_\text{MLE} = \frac{7}{10} = 0.7$$</div>
<p><span class="kw">MLE는 항상 "관측된 빈도 그 자체"</span>다. 직관적이지만, 데이터가 적을 때는 위험하다.</p>

<p><strong>(2) MAP 계산.</strong> Beta(2,2) 사전 분포의 평균 = $2/(2+2) = 0.5$. "동전이 공정할 것 같다"는 약한 믿음을 수식으로 표현한 것이다. 사후 분포:</p>
<div class="prob-formula">$$p(\theta|D) \propto \underbrace{p^7(1-p)^3}_{\text{우도}} \cdot \underbrace{p^{1}(1-p)^{1}}_{\text{Beta(2,2)}} = p^8(1-p)^4 \sim \text{Beta}(9, 5)$$</div>
<p>Beta($\alpha, \beta$)의 최빈값(mode) = $(\alpha-1)/(\alpha+\beta-2)$ 공식을 쓰면:</p>
<div class="prob-formula">$$\hat{p}_\text{MAP} = \frac{9-1}{9+5-2} = \frac{8}{12} = 0.667$$</div>
<p><span class="kw2">사전 분포 Beta(2,2)가 추정값을 0.5 방향으로 "당기는" 효과</span> — MLE(0.7)보다 중앙에 가깝다.</p>

<p><strong>(3) 수렴 분석.</strong> 1000번 시행, 700앞/300뒤: MAP = $701/1002 \approx 0.6996$, MLE = $0.7$. 사전 분포의 가중치(Beta(2,2)의 총합 4)가 데이터(1000)에 비해 무시할 만큼 작아진다. 이것이 <span class="kw3">Bernstein-von Mises 정리</span>의 핵심 — <strong>데이터가 충분히 많으면 사전 분포와 무관하게 MAP → MLE로 수렴한다.</strong> 대규모 딥러닝에서 MLE(교차 엔트로피 손실 등)를 그냥 쓰는 이유가 여기 있다.</p>
...
<p><strong>핵심 세팅.</strong> 잔차 $\epsilon_i \sim \mathcal{N}(0, \sigma^2)$이면 각 관측값의 조건부 확률은 가우시안:</p>
<div class="prob-formula">$$p(y_i | x_i, \theta) = \frac{1}{\sqrt{2\pi\sigma^2}}\exp\!\left(-\frac{(y_i - \theta^T x_i)^2}{2\sigma^2}\right)$$</div>
<p>$n$개 데이터가 독립이므로 전체 로그 우도:</p>
<div class="prob-formula">$$\log L(\theta) = -\frac{n}{2}\log(2\pi\sigma^2) - \frac{1}{2\sigma^2}\sum_{i=1}^n (y_i - \theta^T x_i)^2$$</div>
<p><span class="kw">$\theta$에 관한 최대화 → 앞의 상수(첫째 항) 무시 → $\sum(y_i - \theta^T x_i)^2$ 최소화.</span> 이것이 MSE 손실이다. 따라서 <span class="kw2">$\hat\theta_\text{MLE} = \hat\theta_\text{MSE}$. ∎</span></p>

<p><strong>구체적 수치로 확인.</strong> 데이터 3개: $(1, 2.1), (2, 3.9), (3, 6.2)$, 모델 $y = \theta x$. MSE를 최소화하면 $\hat\theta \approx 2.03$. 이 값이 동시에 잔차가 정규분포를 따른다는 가정 하에서의 MLE이기도 하다.</p>

<p><strong>왜 이것이 중요한가.</strong> MSE를 "당연한 손실"로 쓰는 것은 <span class="kw3">암묵적으로 "잔차가 정규 분포를 따른다"고 가정</span>하는 것이다. 이 가정이 틀리면 MSE는 최적이 아니다:</p>
<ul style="margin:.4rem 0 .6rem 1.2rem;line-height:1.7">
  <li>이상치가 많은 데이터 → 라플라스 분포 가정 → <span class="kw2">MAE(L1 손실)</span>이 더 강건</li>
  <li>이진 분류 → 베르누이 분포 가정 → <span class="kw2">교차 엔트로피 손실</span>이 적절</li>
  <li>포아송 카운트 데이터 → 포아송 분포 가정 → 포아송 손실이 적절</li>
</ul>
<p>손실 함수 선택 = 데이터 생성 모델에 대한 가정이다.</p>
...
<p><strong>MLE의 문제 — 수치로 확인.</strong> 5개 데이터 중 4명 양성 → <span class="kw3">$\hat{p}_\text{MLE} = 4/5 = 0.8$. 그러나 실제 유병률은 1%.</span> MLE는 관측된 5명에만 집중하고, 전체 인구에 대한 도메인 지식을 완전히 무시한다. 진단 양성 환자만 데이터에 포함된 표본 편향 상황에서 특히 심각해진다.</p>

<p><strong>MAP의 해결 — 사전 지식을 수식으로 인코딩.</strong> "유병률 ≈ 1%"를 Beta 사전 분포로 표현한다. Beta($\alpha, \beta$)의 평균 = $\alpha/(\alpha+\beta) = 0.01$이 되려면 $\beta \approx 99\alpha$. 강한 사전 지식을 반영하려면 $\alpha+\beta$를 크게 → Beta(1, 99) 사용.</p>
<div class="prob-formula">$$p(\theta|D) \propto \theta^4(1-\theta)^1 \cdot \theta^0(1-\theta)^{98} = \theta^4(1-\theta)^{99} \sim \text{Beta}(5, 100)$$</div>
<div class="prob-formula">$$\hat{p}_\text{MAP} = \frac{5-1}{5+100-2} = \frac{4}{103} \approx 0.039$$</div>
<p>MLE(0.8)보다 훨씬 낮고, 실제 유병률(0.01)에 훨씬 가깝다. <span class="kw">사전 분포가 강할수록, 데이터가 적을수록 MAP는 사전 지식을 강하게 반영한다.</span></p>

<p><strong>언제 MAP가 필수인가.</strong> <span class="kw2">의료 진단, 법률 판단, 자율주행 안전 시스템</span>처럼 소표본 + 강한 도메인 지식이 있는 분야. 반대로 데이터가 충분히 많으면 사전 분포의 영향이 희석되어 MAP ≈ MLE가 되므로, 대규모 딥러닝에서는 MLE가 실용적으로 충분하다.</p>
...
<p><strong>가우시안에서 mode = mean인 이유.</strong> 가우시안 분포 $\mathcal{N}(\mu, \sigma^2)$의 확률 밀도는 $\mu$를 중심으로 <span class="kw">완전히 좌우 대칭</span>이다. 대칭 분포에서는 최빈값(mode), 평균(mean), 중앙값(median)이 모두 같다. 따라서 가우시안 사후 분포에서 MAP = 사후 평균 = $\mu$.</p>

<p><strong>Beta 사후에서 mode ≠ mean — 수치 예시.</strong></p>
<div class="prob-formula">$$\text{Beta}(\alpha, \beta): \quad \text{mode} = \frac{\alpha-1}{\alpha+\beta-2}, \quad \text{mean} = \frac{\alpha}{\alpha+\beta}$$</div>
<p>예시: Beta(2, 8) — "양성 2건, 음성 8건" 관측에 대응.</p>
<div class="prob-formula">$$\text{mode} = \frac{1}{8} = 0.125, \qquad \text{mean} = \frac{2}{10} = 0.200$$</div>
<p>분포가 왼쪽에 봉우리를 갖고 오른쪽으로 길게 뻗어 있어(right-skewed) mean > mode. <span class="kw3">MAP(= mode = 0.125)가 사후 분포의 "중심"(mean = 0.2)이 아닌 위치를 반환한다.</span></p>

<p><strong>점 추정만 쓸 때의 위험성.</strong> MAP는 단 하나의 파라미터 값만 반환한다. <span class="kw3">사후 분포가 얼마나 퍼져 있는지(불확실한지)를 전혀 전달하지 못한다.</span></p>
<ul style="margin:.4rem 0 .6rem 1.2rem;line-height:1.7">
  <li>MAP = 0.7, 사후 분포가 [0.3, 0.99]에 퍼져 있음 → 매우 불확실, 중요 결정을 내리면 안 됨</li>
  <li>MAP = 0.7, 사후 분포가 [0.68, 0.72]에 집중 → 매우 확실, 결정 가능</li>
</ul>
<p>두 경우 MAP는 같지만 의사결정에서 전혀 다른 행동이 필요하다. <span class="kw2">완전 베이즈(full Bayesian) 접근</span>은 사후 분포 전체를 유지하여 이 불확실성을 다운스트림 결정에 전파한다.</p>
...
<p><strong>교차 엔트로피란?</strong> 정답 레이블 $y$ (원핫 벡터)와 모델 예측 $\hat{y}$가 있을 때: $\mathcal{L} = -\sum_k y_k \log \hat{y}_k$. 원핫에서 정답 클래스만 $y_k = 1$이므로 <span class="kw">$\mathcal{L} = -\log \hat{y}_\text{정답}$</span>. 즉 정답 클래스에 부여한 확률의 음의 로그.</p>

<p><strong>수치 계산.</strong></p>
<div class="prob-formula">$$\mathcal{L}_1 = -\log(0.7) \approx 0.357, \qquad \mathcal{L}_2 = -\log(0.98) \approx 0.020$$</div>
<p>정답 클래스(클래스 2)에 70% → 손실 0.357, 98% → 손실 0.020. <span class="kw2">확신할수록 손실이 0에 가까워진다.</span></p>

<p><strong>카테고리 분포 MLE와 동치 증명.</strong> $K$-클래스 카테고리 분포의 우도: $p(y|\hat{y}) = \prod_k \hat{y}_k^{y_k}$. 음의 로그 우도를 취하면:</p>
<div class="prob-formula">$$\text{NLL} = -\log p(y|\hat{y}) = -\sum_k y_k \log \hat{y}_k = H(y, \hat{y})$$</div>
<p>이것이 정확히 교차 엔트로피다. 따라서 <span class="kw2">분류 모델의 교차 엔트로피 손실 최소화 = 카테고리 분포의 MLE</span>. 분류 문제에서 "왜 교차 엔트로피를 쓰냐"는 질문의 통계적 답이 여기 있다.</p>

<p><strong>확신할수록 손실이 작아지는 이유.</strong> <span class="kw">$-\log(x)$는 $x \to 1$에서 0으로 수렴, $x \to 0$에서 $+\infty$로 발산</span>한다. 정답 클래스에 확률 0.99를 부여하면 $-\log(0.99) \approx 0.01$ (거의 0), 확률 0.01을 부여하면 $-\log(0.01) \approx 4.6$ (매우 큰 페널티). 이 <span class="kw3">비대칭적 페널티 구조</span>가 모델이 정답에 확신을 키우도록 학습을 강제한다.</p>
