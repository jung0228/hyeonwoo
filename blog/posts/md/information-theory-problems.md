...
<p><strong>엔트로피 공식.</strong> 이진 확률 변수에서 $H = -p\log_2 p - (1-p)\log_2(1-p)$. 단위: bit. 각 항 $-p_i \log_2 p_i$는 사건 $i$가 일어날 때의 <span class="kw">"놀람 정도 = $\log_2(1/p_i)$"에 확률을 가중한 기댓값</span>이다.</p>
<div class="prob-formula">$$H(0.5) = -0.5\log_2 0.5 - 0.5\log_2 0.5 = 0.5 \times 1 + 0.5 \times 1 = 1 \text{ bit}$$
$$H(0.9) = -0.9\log_2 0.9 - 0.1\log_2 0.1 \approx 0.9(0.152) + 0.1(3.322) \approx 0.469 \text{ bit}$$</div>
<p><span class="kw2">공정한 동전의 엔트로피(1 bit)가 편향된 동전(0.469 bit)의 약 2.1배</span>다.</p>

<p><strong>직관 — 왜 편향된 동전의 엔트로피가 낮은가.</strong> $p=0.9$ 동전을 던지기 전, 이미 "앞면이 나올 것 같다"고 꽤 확신한다. 결과를 알아도 새로 얻는 정보의 양이 적다. 반면 공정한 동전은 완전히 예측 불가능 — 결과를 알았을 때 최대의 정보를 얻는다. <span class="kw">엔트로피 = "결과를 알기 전 평균적으로 얼마나 놀랄 것인가" = 평균 정보량</span>.</p>

<p><strong>날씨 예보의 함의.</strong> "맑을 확률 99%"는 $H \approx 0.08$ bit — 거의 확신이 있으므로 우산 결정이 쉽다. "맑을 확률 50%"는 <span class="kw3">$H = 1$ bit — 완전히 불확실하므로 예보 자체가 의사결정에 도움이 안 된다.</span> 기상 예보 품질은 사전 불확실성에서 엔트로피를 얼마나 줄이는지(정보 이득)로 수치화할 수 있다.</p>
...
<p><strong>전략 (A) 손실.</strong> 레이블이 0이든 1이든 항상 $\hat{y}_\text{정답} = 0.5$. 기대 교차 엔트로피 손실:</p>
<div class="prob-formula">$$\mathcal{L}_A = -\log(0.5) = 1 \text{ bit} \quad (\text{또는 } \ln 2 \approx 0.693 \text{ nats})$$</div>
<p><span class="kw">이것이 레이블 엔트로피 $H(Y) = 1$ bit와 정확히 같다 — 우연이 아니다.</span></p>

<p><strong>전략 (B) 손실.</strong> 완벽한 모델이라면 정답 클래스에 확률 1을 부여 → $-\log(1) = 0$. 기대 손실 = $0$. 단, 소프트맥스 출력은 지수 함수이므로 <span class="kw3">실제로 확률이 정확히 1.0에 도달할 수 없다</span> — 교차 엔트로피 손실 = 0은 이론적 극한이다.</p>

<p><strong>핵심 통찰 — 무엇을 줄일 수 있는가.</strong> 교차 엔트로피는 다음과 같이 분해된다:</p>
<div class="prob-formula">$$H(p, q) = H(p) + D_{KL}(p\|q)$$</div>
<ul style="margin:.4rem 0 .6rem 1.2rem;line-height:1.7">
  <li><span class="kw3">$H(p)$ = 레이블 자체의 불확실성</span> → 데이터가 고정되어 있으므로 어떤 모델로도 줄일 수 없다</li>
  <li><span class="kw2">$D_{KL}(p\|q)$ = 모델이 레이블 분포에서 얼마나 벗어나 있는지</span> → 학습으로 줄일 수 있다</li>
</ul>
<p>전략 (A)의 손실 = $H(Y) + D_{KL}$ (최악). 전략 (B)의 손실 = $H(Y)$ (이론적 하한). 딥러닝 학습이 실제로 하는 일은 $D_{KL}$ 부분을 줄이는 것이다.</p>
...
<p><strong>KL Divergence 공식.</strong> $D_{KL}(P\|Q) = \sum_x P(x) \log \frac{P(x)}{Q(x)}$. "실제 분포 $P$로 생성된 데이터에 $Q$ 기반 코드를 썼을 때의 평균 추가 비트 수". 0이면 두 분포가 같다는 뜻.</p>

<p><strong>계산.</strong></p>
<div class="prob-formula">$$D_{KL}(P\|Q) = 0.9\ln\frac{0.9}{0.5} + 0.1\ln\frac{0.1}{0.5} \approx 0.9(0.588) + 0.1(-1.609) \approx 0.368 \text{ nats}$$
$$D_{KL}(Q\|P) = 0.5\ln\frac{0.5}{0.9} + 0.5\ln\frac{0.5}{0.1} \approx 0.5(-0.588) + 0.5(1.609) \approx 0.511 \text{ nats}$$</div>
<p><span class="kw3">두 값이 다르다 — KL Divergence는 비대칭이다. ($D_{KL}(P\|Q) \neq D_{KL}(Q\|P)$)</span></p>

<p><strong>비대칭성의 직관.</strong></p>
<ul style="margin:.4rem 0 .6rem 1.2rem;line-height:1.7">
  <li>$D_{KL}(P\|Q)$: 실제 분포 $P$ (90% 확신)로 샘플을 생성하는데, $Q$ (50-50 가정)로 코딩하면 비효율. $P$가 90%를 할당하는 클래스 0에 $Q$는 겨우 50% — 이 차이가 비효율을 만든다.</li>
  <li>$D_{KL}(Q\|P)$: 균등 분포 $Q$의 관점에서 $P$를 쓸 때의 비효율. $Q$는 클래스 1에 50%를 기대하는데 $P$는 10%만 준다 — 클래스 1에 대한 큰 페널티.</li>
</ul>

<p><strong>"근사 비효율성" 해석과 실용적 함의.</strong> $D_{KL}(P\|Q) = 0.368$ nats는 실제 분포가 $P$인데 $Q$ 기반 코드를 쓰면 샘플당 0.368 nats의 추가 정보가 필요하다는 의미다. <span class="kw">이 비대칭성이 KL이 진정한 "거리"가 아닌 이유</span>이고, <span class="kw2">VAE에서 어느 방향 KL을 쓰는지가 모델 행동에 큰 차이를 만드는 이유</span>다 (forward KL → mean-seeking, reverse KL → mode-seeking).</p>
...
<ul style="margin:.4rem 0 .6rem 1.2rem;line-height:1.7">
  <li><span class="kw">조건부 엔트로피 $H(Y|X)$</span> = 특성 $X$를 알고 난 뒤 레이블 $Y$의 잔여 불확실성. $H(Y|X) = \sum_x p(x) H(Y|X=x)$</li>
  <li><span class="kw2">상호 정보량 $I(X;Y) = H(Y) - H(Y|X)$</span> = 특성 $X$를 알았을 때 $Y$의 불확실성이 얼마나 줄어드는가</li>
</ul>
<p>$I(X;Y)$가 클수록 $X$가 $Y$에 대한 정보를 많이 담고 있다. 결정 트리는 매 분기에서 $I(X;Y)$ 최대 특성을 선택한다.</p>

<p><strong>수치 예시 — 특성 A.</strong> 10개 샘플, 레이블 50/50. $H(Y) = 1$ bit. 특성 A: $X=0$이면 4양성/1음성(각 5개씩), $X=1$이면 1양성/4음성.</p>
<div class="prob-formula">$$H(Y|A=0) = -\frac{4}{5}\log_2\frac{4}{5} - \frac{1}{5}\log_2\frac{1}{5} \approx 0.722 \text{ bit}$$
$$H(Y|A) = \frac{5}{10}(0.722) + \frac{5}{10}(0.722) = 0.722 \text{ bit}$$
$$I(A;Y) = 1 - 0.722 = 0.278 \text{ bit}$$</div>

<p><strong>수치 예시 — 특성 B.</strong> $X=0$이면 5양성/0음성, $X=1$이면 0양성/5음성. 완벽한 분리.</p>
<div class="prob-formula">$$H(Y|B=0) = H(Y|B=1) = 0, \quad I(B;Y) = 1 - 0 = 1 \text{ bit}$$</div>
<p><span class="kw2">특성 B가 레이블을 완벽히 분리 → 정보 이득 최대(= $H(Y)$)</span>. 반면 레이블과 전혀 무관한 특성은 $H(Y|X) = H(Y)$이므로 $I = 0$이 된다.</p>
...
<p><strong>분해 유도 — 한 줄씩 풀어서.</strong></p>
<div class="prob-formula">$$H(p,q) = -\sum_x p(x)\log q(x)$$
$$= -\sum_x p(x)\log p(x) + \sum_x p(x)\log p(x) - \sum_x p(x)\log q(x)$$
$$= H(p) + \sum_x p(x)\log\frac{p(x)}{q(x)} = H(p) + D_{KL}(p\|q)$$</div>
<p>두 번째 줄에서 $-\sum p(x)\log p(x) = H(p)$를 더하고 빼는 트릭을 썼다. ∎</p>

<p><strong>(1) 최솟값 증명.</strong> <span class="kw">깁스 부등식: $D_{KL}(p\|q) \geq 0$</span>, 등호는 $p = q$일 때만 성립. (증명: 로그의 볼록성으로 Jensen 부등식 적용.) 따라서:</p>
<div class="prob-formula">$$H(p, q) = H(p) + D_{KL}(p\|q) \geq H(p)$$</div>
<p><span class="kw2">교차 엔트로피의 최솟값 = 레이블 엔트로피 $H(p)$, 등호는 $q = p$일 때</span>. 즉 모델이 완벽히 데이터 분포를 학습했을 때 달성된다. ∎</p>

<p><strong>(2) 딥러닝 학습이 실제로 하는 일.</strong> 훈련 중 레이블 분포 $p$ (데이터)는 고정 → $H(p)$는 상수. 바뀌는 것은 모델 $q_\theta$뿐. 따라서:</p>
<div class="prob-formula">$$\min_\theta H(p, q_\theta) = \min_\theta \bigl[H(p) + D_{KL}(p \| q_\theta)\bigr] = \min_\theta D_{KL}(p \| q_\theta)$$</div>
<p><span class="kw2">교차 엔트로피 손실 최소화 = $D_{KL}(p \| q_\theta)$ 최소화 = 모델 분포를 데이터 분포에 가깝게 만들기.</span> 이것이 딥러닝 학습의 정보이론적 본질이다.</p>
