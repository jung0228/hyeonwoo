...
<div class="prob-q">$A^2 = A$를 만족하는 멱등 행렬(idempotent matrix)의 고유값이 0 또는 1만 가능함을 증명하라. 이것이 사영(projection) 연산의 직관과 어떻게 연결되는지 설명하고, 딥러닝에서 자기 자신에게 두 번 곱해도 같은 연산이 이 성질을 어떻게 활용하는지 논하라.</div>
<button class="prob-toggle" onclick="tp(this)">모범 답안 보기 ▾</button>
<div class="prob-ans">

<p><strong>멱등(idempotent)이 뭔가?</strong> "같은 것을 두 번 해도 한 번 한 것과 같다"는 뜻이다. 수식으로는 $A^2 = A \cdot A = A$. 구체적인 예:</p>
<div class="prob-formula">$$A = \begin{pmatrix}1&0\\0&0\end{pmatrix}, \quad A^2 = \begin{pmatrix}1&0\\0&0\end{pmatrix}\begin{pmatrix}1&0\\0&0\end{pmatrix} = \begin{pmatrix}1&0\\0&0\end{pmatrix} = A \checkmark$$</div>
<p>이 행렬은 어떤 벡터 $\begin{pmatrix}x\\y\end{pmatrix}$든 $\begin{pmatrix}x\\0\end{pmatrix}$으로 만든다 — $x$축에 "그림자를 드리우는(사영하는)" 연산이다.</p>

<p><strong>사영(projection)이 뭔가?</strong> 벽에 물체를 비추면 바닥에 그림자가 생긴다. 그림자를 다시 비춰봤자 그림자는 그대로다. 이것이 사영의 핵심: <strong>한 번 사영한 것을 또 사영해도 변하지 않는다</strong>. 수식으로 $P(Pv) = Pv$, 즉 $P^2 = P$.</p>

<p><strong>증명.</strong> 고유쌍 $(\lambda, v)$, 즉 $Av = \lambda v$를 가정한다. $A^2$을 고유벡터에 적용하면:</p>
<div class="prob-formula">$$A^2 v = A(Av) = A(\lambda v) = \lambda \cdot Av = \lambda \cdot \lambda v = \lambda^2 v$$</div>
<p>그런데 $A^2 = A$이므로 $A^2 v = Av = \lambda v$. 따라서 $\lambda^2 v = \lambda v$, 즉 $(\lambda^2 - \lambda)v = 0$. $v \neq 0$이므로:</p>
<div class="prob-formula">$$\lambda^2 - \lambda = \lambda(\lambda - 1) = 0 \implies \lambda = 0 \text{ 또는 } \lambda = 1 \quad \checkmark$$</div>

<p><strong>고유값의 의미.</strong> $\lambda = 1$인 고유벡터 = "사영해도 그대로 남는" 벡터 (이미 그림자 평면 위에 있는 벡터). 위 예시에서 $\begin{pmatrix}1\\0\end{pmatrix}$은 $A\begin{pmatrix}1\\0\end{pmatrix} = \begin{pmatrix}1\\0\end{pmatrix}$ → 고유값 1. $\lambda = 0$인 고유벡터 = "사영하면 사라지는" 벡터 ($y$축 방향, 바닥에 수직인 빛). $A\begin{pmatrix}0\\1\end{pmatrix} = \begin{pmatrix}0\\0\end{pmatrix}$ → 고유값 0.</p>

<p><strong>딥러닝 연결.</strong> <strong>Dropout</strong>은 학습 시 뉴런을 50% 확률로 끈다. 마스킹 행렬 $M$은 대각선이 0 또는 1인 행렬 → $M^2 = M$ (멱등). 같은 마스크를 두 번 적용해도 결과가 같다. <strong>Attention mask</strong>(미래 토큰 차단)도 마찬가지다. 이런 멱등 연산은 수치적으로 안전하다 — 여러 번 반복해도 결과가 폭발하거나 0이 되지 않는다.</p>
...
<p><strong>직교 행렬이 뭔가?</strong> $Q^TQ = I$를 만족하는 행렬이다. 직관적으로는 <strong>"회전만 하는 행렬"</strong>이다. 2D 회전 행렬이 가장 대표적인 예시:</p>
<div class="prob-formula">$$Q = \begin{pmatrix}\cos\theta & -\sin\theta \\ \sin\theta & \cos\theta\end{pmatrix}, \quad Q^T Q = \begin{pmatrix}\cos^2\theta+\sin^2\theta & 0 \\ 0 & \sin^2\theta+\cos^2\theta\end{pmatrix} = I \checkmark$$</div>
<p>$\theta = 90°$로 예를 들면 $Q = \begin{pmatrix}0&-1\\1&0\end{pmatrix}$. 벡터 $\begin{pmatrix}1\\0\end{pmatrix}$을 곱하면 $\begin{pmatrix}0\\1\end{pmatrix}$ — 90도 회전, 길이는 그대로.</p>

<p><strong>$Q^TQ = I$의 의미.</strong> $Q$의 각 열벡터 $q_i$에 대해 $q_i \cdot q_i = 1$ (길이 1), $q_i \cdot q_j = 0$ ($i \neq j$, 서로 수직). 즉 열벡터들이 서로 수직이고 길이가 1인 "정규직교 기저"다.</p>

<p><strong>증명: $|\lambda| = 1$.</strong> 고유쌍 $Qv = \lambda v$를 가정. 변환 전후의 벡터 길이를 비교한다:</p>
<div class="prob-formula">$$\|Qv\|^2 = (Qv)^T(Qv) = v^T \underbrace{Q^T Q}_{= I} v = v^T v = \|v\|^2$$</div>
<p>한편 $Qv = \lambda v$이므로 $\|Qv\|^2 = \|\lambda v\|^2 = |\lambda|^2 \|v\|^2$. 두 식을 같다고 놓으면 $|\lambda|^2 \|v\|^2 = \|v\|^2$. $\|v\|^2 > 0$이므로 $|\lambda|^2 = 1$, 즉 $|\lambda| = 1$. ∎</p>

<p><strong>직관.</strong> 모든 고유값의 크기가 1이라는 것은 "어떤 방향으로도 벡터를 늘리거나 줄이지 않는다"는 뜻이다. 직교 행렬은 길이를 보존하는 회전/반사만 한다. $|\lambda| > 1$이면 해당 방향 벡터가 커지고, $|\lambda| < 1$이면 줄어드는데, 직교 행렬은 이런 일이 없다.</p>

<p><strong>딥러닝: 그래디언트 소실/폭발 문제.</strong> 신경망은 여러 레이어가 행렬 곱으로 연결된다. 예를 들어 3개 레이어: $h = W_3 W_2 W_1 x$. 역전파 시 그래디언트는 이 행렬들을 거꾸로 통과한다. 만약 $W_i$의 고유값이 모두 2.0이면:</p>
<div class="prob-formula">$$\text{그래디언트 크기} \propto 2^{100} \approx 10^{30} \quad \text{(100층이면 폭발)}$$</div>
<p>반면 모두 직교 행렬이면 $|\lambda| = 1$이므로 100층을 거쳐도 그래디언트 크기가 그대로 보존된다. <strong>직교 초기화</strong>는 학습 시작 시 $W_i$를 직교에 가깝게 잡아서 이 안정성을 초반에 확보한다. 학습이 진행되면서 직교성이 깨지지만, 안정적인 출발이 학습 전체에 좋은 영향을 준다.</p>
...
<div class="prob-q">$\text{tr}(A) = \sum_i \lambda_i$임을 이용하여, PCA에서 상위 $k$개의 주성분이 설명하는 분산의 비율을 공분산 행렬의 고유값으로 표현하라. 또한 잔차 연결(residual connection)이 있는 네트워크 $F(x) = x + f(x)$에서 야코비안(Jacobian)의 고유값이 그래디언트 흐름에 왜 유리한지 설명하라.</div>
<button class="prob-toggle" onclick="tp(this)">모범 답안 보기 ▾</button>
<div class="prob-ans">

<p><strong>Trace(대각합)가 뭔가?</strong> 행렬의 대각선 원소들의 합이다. 예:</p>
<div class="prob-formula">$$A = \begin{pmatrix}3&1\\1&3\end{pmatrix} \implies \text{tr}(A) = 3 + 3 = 6$$</div>
<p>이 행렬의 고유값은 4와 2이고, $4 + 2 = 6 = \text{tr}(A)$. 이것이 "$\text{tr}(A) = \sum \lambda_i$"다. 수치 예시로 확인이 됐다.</p>

<p><strong>PCA에서 Trace의 역할.</strong> 데이터의 각 변수(특성)의 분산을 모두 더한 것이 "총 분산"이다. 예를 들어 신장과 체중 데이터라면 총 분산 = (신장 분산) + (체중 분산). 공분산 행렬 $C$는 대각선에 각 변수의 분산이 있으므로 총 분산 = $\text{tr}(C) = \sum \lambda_i$.</p>
<p>PCA는 이 분산을 고유값이 큰 방향(주성분)부터 설명한다. 상위 $k$개 주성분이 설명하는 비율:</p>
<div class="prob-formula">$$\text{설명된 분산 비율} = \frac{\lambda_1 + \lambda_2 + \cdots + \lambda_k}{\lambda_1 + \lambda_2 + \cdots + \lambda_d} = \frac{\sum_{i=1}^k \lambda_i}{\text{tr}(C)}$$</div>
<p>예: 고유값이 $[10, 4, 1]$이면 총 분산 = 15. 상위 2개 주성분: $(10+4)/15 = 93\%$ 설명. 첫 번째 주성분 하나로도 $10/15 = 67\%$. PCA에서 "주성분을 몇 개 쓸지" 결정할 때 이 비율을 보고 보통 90~95% 이상이 되는 $k$를 선택한다.</p>

<p><strong>야코비안(Jacobian)이 뭔가?</strong> 야코비안은 다변수 함수의 "미분 행렬"이다. 1변수 함수 $y = f(x)$의 미분이 $dy/dx$이듯, 다변수 함수 $y = f(x)$ ($x$가 벡터)의 미분은 행렬이 된다: $J_{ij} = \partial y_i / \partial x_j$. 역전파에서 그래디언트는 이 야코비안을 통해 전파된다.</p>

<p><strong>잔차 연결(Residual Connection)과 고유값.</strong> 보통 레이어: $h = f(x)$. 잔차 연결: $h = x + f(x)$. 야코비안은:</p>
<div class="prob-formula">$$J_F = I + J_f$$</div>
<p>$f$의 야코비안 고유값을 $\mu$라 하면, $F = I + J_f$의 고유값은 $1 + \mu$. 만약 $f$가 학습 초반에 거의 0에 가깝다면 ($\mu \approx 0$), $F$의 고유값 $\approx 1$. 즉 그래디언트가 이 레이어를 통과할 때 크기가 거의 보존된다.</p>

<p>반면 일반 레이어 $h = f(x)$는 $J_f$의 고유값 자체가 그래디언트 배율이 된다. 100층이면 $\mu^{100}$ — $|\mu| < 1$이면 $0$으로 소멸, $|\mu| > 1$이면 폭발. <strong>잔차 연결은 "최소 1의 패스웨이"를 만들어 그래디언트가 레이어를 건너뛰어 흐를 수 있게 한다.</strong> ResNet이 1000층 이상도 학습 가능한 이유가 여기 있다.</p>
...
<p><strong>이차형식(quadratic form) $v^TAv$가 뭔가?</strong> 벡터 $v$와 행렬 $A$로 만드는 스칼라(숫자 하나)다. 결과가 항상 양수인지($> 0$), 또는 0이 될 수도 있는지($\geq 0$)에 따라 행렬을 분류한다. 구체적 예:</p>
<div class="prob-formula">$$A = \begin{pmatrix}2&0\\0&3\end{pmatrix}, \; v = \begin{pmatrix}1\\1\end{pmatrix} \implies v^TAv = \begin{pmatrix}1&1\end{pmatrix}\begin{pmatrix}2\\3\end{pmatrix} = 5 > 0$$</div>
<p>어떤 $v$를 넣어도 양수가 나온다면 → PD (Positive Definite, 양의 정부호).</p>

<p><strong>PD vs PSD 정의.</strong></p>
<p><span class="kw">PD(Positive Definite)</span>: 모든 $v \neq 0$에 대해 $v^TAv > 0$. ↔ 모든 고유값이 <strong>양수</strong> ($\lambda_i > 0$). 직관: 어느 방향으로 가도 "그릇 모양"처럼 위로 올라간다.</p>
<p><span class="kw2">PSD(Positive Semi-Definite)</span>: 모든 $v \neq 0$에 대해 $v^TAv \geq 0$. ↔ 모든 고유값이 <strong>비음수</strong> ($\lambda_i \geq 0$). 직관: 대부분 위로 올라가되, 일부 방향은 "평평한(0이 되는)" 것이 허용된다.</p>
<div class="prob-formula">$$B = \begin{pmatrix}1&0\\0&0\end{pmatrix}: \quad v^TBv = v_1^2 \geq 0, \text{ 단 } v = \begin{pmatrix}0\\1\end{pmatrix}\text{이면 } 0 \implies \text{PSD이지만 PD 아님}$$</div>

<p><strong>공분산 행렬이 PSD인 증명.</strong> 데이터 행렬 $X$ ($n$개 샘플, $d$개 특성)의 공분산 $C = \frac{1}{n}X^TX$. 임의 벡터 $v$에 대해:</p>
<div class="prob-formula">$$v^T C v = \frac{1}{n} v^T X^T X v = \frac{1}{n} \|Xv\|^2 \geq 0$$</div>
<p>$\|Xv\|^2$는 길이의 제곱이므로 항상 $\geq 0$. ∴ PSD. ∎</p>

<p><strong>왜 PD가 아닐 수 있는가 — 구체적 예시.</strong> 키(cm)와 키(inch)를 특성으로 쓴다고 하자. 두 특성이 완벽히 비례 관계 ($\text{inch} = \text{cm}/2.54$). 이러면 데이터가 2차원 공간의 1차원 직선 위에만 있다. 그 직선에 수직인 방향 $v$으로 데이터를 투영하면 $Xv = 0$이 되어 $v^TCv = 0$. 고유값 중 하나가 0 → PSD이지만 PD 아님.</p>

<p>또 다른 경우: 데이터가 3개(샘플)인데 특성이 100개(차원)라면, 데이터 행렬 $X$의 rank ≤ 3. 100×100 공분산 행렬의 고유값 97개는 반드시 0 → PD 불가능.</p>

<p><strong>실전 문제.</strong> 가우시안 분포의 확률밀도함수에는 $C^{-1}$과 $\det(C)$가 들어간다. $C$가 singular(고유값 중 0이 있으면 $\det = 0$)이면 역행렬이 존재하지 않고 분모가 0이 되어 <strong>PDF 자체가 정의 불가</strong>. 해결책:</p>
<p>(a) <strong>정규화:</strong> $C' = C + \epsilon I$ — 모든 고유값에 $\epsilon > 0$을 더해 강제로 PD. ($\epsilon = 10^{-6}$ 정도의 아주 작은 값.)</p>
<p>(b) <strong>PCA 전처리:</strong> 0인 고유값에 해당하는 방향을 버리고 rank만큼만 남긴다.</p>
<p>(c) <strong>특성 선택:</strong> 선형 종속인 특성을 제거한다.</p>
