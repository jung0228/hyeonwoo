...
desc: Deep Research의 정의·파이프라인·목적 task를 먼저 정리하고, 그 위에서 personalization 연구가 프레임워크·memory·benchmark·평가 방법론으로 이어지는 흐름을 5편의 논문으로 정리한다.
tags: [Agent, LLM]
date: Jun 2026
...
이 글은 두 갈래를 정리한다. 먼저 **Deep Research(DR)가 정확히 무엇을 하는 분야인지** — 입출력 과정, 목적 task, RAG와의 차이 — 를 서베이로 단단히 깔고, 그 위에서 **personalization(개인화)이 어떻게 secondary 과제에서 핵심 연구 방향으로 떠오르는지**를 5편의 논문으로 따라간다.
...
**무엇이 달라지는가 — 그리고 얼마나.** 개인화를 붙이면 보고서의 말투만 바뀌는 게 아니라 **information filtering(무엇을 포함/배제), reasoning strategy(어떤 관점으로 분석), recommendation(무엇을 권함)** 셋이 다 바뀐다. 영향도는 측정된다 — Part 2의 PDR-Bench 실험에서 같은 시스템도 **사용자 정보를 줄수록(Task Only &lt; Task+Context &lt; Task+Persona)** 점수가 단조 증가하며, 특히 명시적 persona를 줬을 때 가장 크게 오른다. 즉 개인화는 "있으면 좋은" 장식이 아니라 **출력 효용을 좌우하는 변수**다.
...
<figcaption><strong>Figure 3</strong> — <strong>historical data</strong>("who the user is" — 식별자·인구통계·과거 상호작용·feedback)가 행동을 Guide하고, <strong>interaction data</strong>(실행 중의 explicit feedback[교정·평가]·implicit feedback[반응 지연·tone·감정])가 그 기록을 Refine한다. 출처: 같은 논문 Fig 2.</figcaption>
</figure>
...
<figcaption><strong>Figure 6</strong> — "이번 주말 활동 추천?"에 대한 두 경로. (위) O-Mem의 <strong>user-centric</strong> 방식 — 흩어진 단서를 correlate해 "아파서 시험을 망침"을 반영 → "집에서 공부하라"(✓). (아래) 기존 <strong>semantic grouping</strong> — 의미로만 묶어 핵심 맥락을 놓침 → "농구나 배드민턴 하라"(✗). 출처: O-Mem (arXiv:2511.13593), Fig 1.</figcaption>
</figure>
...
<figcaption><strong>Figure 7</strong> — 성능(F1) 대 비용. <strong>O-Mem(초록 ▽)</strong>이 좌상단 — 가장 높은 F1을 (좌) 가장 짧은 average time delay, (우) 가장 낮은 token cost로 달성한다. LangMem(주황)은 비슷한 F1을 내지만 시간·토큰을 훨씬 많이 쓴다. 출처: O-Mem (arXiv:2511.13593), figure/two.png.</figcaption>
</figure>
...
<figcaption><strong>Figure 8</strong> — 벤치 구성 파이프라인. (좌) 10 도메인 × 도메인 전문가→위원회 검토 → 50 task. (중) 25 volunteer가 structured schema에 실제 정보 기입(explicit persona) + 폰 앱으로 일상·대화 수집(personalized context). (우) task 선택 + 위원회 검토 → 250 task-user 쌍. 출처: PDR-Bench (arXiv:2509.25106), Fig 2.</figcaption>
</figure>
...
<figcaption><strong>Figure 9</strong> — PQR 평가 프레임워크. <strong>P</strong>: Judge LLM이 dimension weight 할당(GOAL/CONT/PRES/ACTI) → task·persona별 sub-criterion 생성 → 0–10 채점. <strong>Q</strong>: Depth·Coherence·Clarity 동일 방식. <strong>R</strong>: claim 추출 → web search로 출처 검증 → FA·CC. 출처: 같은 논문 Fig 3.</figcaption>
</figure>
...
<figcaption><strong>Figure 10</strong> — 정보 가용성 ablation. 모든 차원(P-Score·GOAL·CONT·PRES·ACTI)에서 <strong>Only Task &lt; w/Context &lt; w/Persona</strong>로 단조 증가 — 개인화 신호를 줄수록, 특히 명시적 persona일수록 점수가 오른다. 출처: 같은 논문 Fig (실험).</figcaption>
</figure>
...
<figcaption><strong>Figure 12</strong> — nine errors별로, 네 LLM judge(Gemini-2.5 Flash·GPT-4.1·Claude-4 Sonnet·DeepSeek-V3)의 사용자 만족 예측 정확도. 대부분 <strong>majority-class baseline(점선)에 못 미치거나 비슷</strong> — 강력한 judge로도 이 차원들을 못 잡는다는 직접 증거. 출처: 같은 논문 Fig (simulation).</figcaption>
</figure>
...
<div class="callout">
<strong>이 논문의 위치 —</strong> ③이 만든 측정 도구(LLM judge)가 <strong>무엇을 못 재는지를 real user로 폭로</strong>한다. 자동 평가는 misleading success signal을 줄 수 있다 — 정량 지표가 다 통과해도 실제 사용자는 불만일 수 있다. 이는 ①이 open problem으로 든 "evaluation" 항목의 실증이며, Part 4의 출발점이다.
</div>
