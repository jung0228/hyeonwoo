...
**둘째**, MomentSeeker IMS에서 LanguageBind R@1이 4.8이라는 숫자는 기회다. 태스크는 정의됐지만 **학습에 쓸 데이터가 없어서** 성능이 낮다. TCVP가 real-world IMS 학습 데이터를 제공하면 직접적인 기여가 된다.

**셋째**, MINOTAUR와 EAGLE이 Ego4D에서만 이미지 쿼리를 다뤘다는 한계는 TCVP에게 포지셔닝 기회다. **일반 YouTube 영상 + real user intent + 이미지 쿼리**의 교집합은 아직 비어있다.

**넷째**, HUD가 지적한 것처럼 이미지 쿼리는 under-specified하다. 이 모호성을 댓글 텍스트가 해소하는 TCVP의 `(q_t, q_v)` 구조가 이 문제에 대한 자연스러운 답이 된다.
