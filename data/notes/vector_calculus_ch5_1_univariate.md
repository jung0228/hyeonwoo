...
## 2. ⚔️ Section 5.1.1: Taylor Series (테일러 급수와 테일러 다항식)


### 📌 1. 테일러 다항식과 테일러 급수의 정의 (Definitions 5.3 ~ 5.4 & Eq 5.7~5.8)

복잡한 비선형 함수 $f(x)$ 를 특정 점 $x_0$ 근방에서 다항함수들의 무한합으로 근사/표현하는 기법입니다:

1. $n$차 테일러 다항식 (Taylor Polynomial: Definition 5.3 & Eq 5.7):
   $$T_n(x) := \sum_{k=0}^n \frac{f^{(k)}(x_0)}{k!} (x - x_0)^k$$
   (여기서 $f^{(k)}(x_0)$ 는 점 $x_0$ 에서의 $k$계 도함수이며, $0! = 1, (x-x_0)^0 = 1$ 로 정의합니다.)

2. 테일러 급수 (Taylor Series: Definition 5.4 & Eq 5.8):
   무한 번 미분 가능한 매끄러운 함수($f \in C^\infty$)에 대해 $n \to \infty$ 로 확장한 무한급수:
   $$T_\infty(x) := \sum_{k=0}^\infty \frac{f^{(k)}(x_0)}{k!} (x - x_0)^k$$
   - 매클로린 급수 (Maclaurin Series): $x_0 = 0$ 을 중심으로 전개한 특수한 테일러 급수.
   - 해석적 함수 (Analytic Function): 자신의 테일러 급수와 함수값이 완벽히 일치하는 함수 ($f(x) = T_\infty(x)$).


### 📌 2. 다항식의 테일러 전개 전수 계산 (Example 5.3)
...
### 📌 3. 삼각함수의 매클로린 급수 전개 (Example 5.4 & Figure 5.4)
...
- 테일러 급수 ($T_\infty(x) = \sum \frac{f^{(k)}(x_0)}{k!}(x-x_0)^k$): 임의의 매끄러운 비선형 함수를 다항함수들의 무한합으로 근사 분해하는 도구입니다.
...
- 1차 테일러 근사(경사하강법) vs 2차 테일러 근사(뉴턴-랩슨법 / Second-order Optimization):
  - 1차 근사 ($f(x) \approx f(x_0) + f'(x_0)(x-x_0)$): 기울기만 사용하므로 계산량이 $O(D)$ 로 매우 저렴하지만, 곡률을 몰라 학습률 설정에 민감합니다.
  - 2차 근사 ($f(x) \approx f(x_0) + f'(x_0)(x-x_0) + \frac{1}{2}f''(x_0)(x-x_0)^2$): 곡률(헤시안)을 고려해 최적점으로 한 번에 도약하지만, 2차 미분 계산량이 $O(D^2 \sim D^3)$ 로 급증합니다.
