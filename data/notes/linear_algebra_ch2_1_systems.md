...
수학에서 수학적 대상(Objects)을 정의하고 이들을 조작하는 규칙의 집합을 Algebra(대수학)라 하며, Linear Algebra(선형대수학)는 벡터와 벡터를 조작하는 대수 규칙을 연구하는 학문입니다.

본 교재에서는 일반적인 벡터를 굵은 글씨 $\mathbf{x}, \mathbf{y}$ 로 표기하며, "더하기(Addition)가 가능하고 스칼라 곱(Scalar Multiplication)을 했을 때 동일한 종류의 대상이 되는 모든 오브젝트"를 벡터로 정의합니다.

### 📌 벡터의 4가지 예시 (Figure 2.1)
1. Geometric Vectors (기하학적 벡터):
   - 방향과 크기를 가지는 화살표/유향 선분 ($\vec{x}, \vec{y}$).
   - $\vec{x} + \vec{y} = \vec{z}$ 이며 스칼라 배 $\lambda \vec{x}$ 역시 동일한 유향 선분입니다.
2. Polynomials (다항식):
...
회사에서 자원 $R_1, \dots, R_m$을 사용하여 제품 $N_1, \dots, N_n$을 생산합니다.
- 제품 $N_j$ 1단위를 생산하는 데 자원 $R_i$가 $a_{ij}$ 단위만큼 필요함.
- 사용 가능한 총 자원이 $b_i$ 단위일 때, 자원을 남김없이 소비하는 최적 생산량 $x_1, \dots, x_n$을 구하는 계획입니다.
...
실수 선형방정식계의 해는 [1] 해가 없음, [2] 정확히 1개의 유일해, [3] 무수히 많은 해 3가지 경우만 존재합니다.
...
2x_1 + 3x_3 &= 1 \quad (3)
\end{aligned}$$
- 풀이: (1)식과 (2)식을 더하면 $2x_1 + 3x_3 = 5$가 됩니다. 하지만 이는 (3)번 식의 $2x_1 + 3x_3 = 1$ 과 모순($5 = 1$)되므로 해가 존재하지 않습니다.
...
- 결론: 오직 $(1, 1, 1)$ 만이 유일한 해(Unique Solution)가 됩니다.
...
2x_1 + 3x_3 &= 5 \quad (3)
\end{aligned}$$
- 풀이: (1)+(2)=(3) 이므로 (3)번 식은 중복(Redundancy)되어 생략 가능함. (1)과 (2)로부터 $2x_1 = 5 - 3x_3$, $2x_2 = 1 + x_3$를 얻습니다.
...
- 2차원 평면 ($\mathbb{R}^2$): 각 선형방정식은 $x_1 x_2$-평면 상의 직선(Line)을 정의합니다. 해집합은 이 직선들의 교점(Intersection)임.
...
방정식계 (2.3)을 계수 벡터들의 선형 결합 형태로 컴팩트하게 표기합니다:
...
이를 행렬-벡터 곱셈 형태 $Ax = b$ 로 압축합니다 (Equation 2.10):
