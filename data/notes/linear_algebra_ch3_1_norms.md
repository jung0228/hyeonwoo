...
벡터 공간 $V$ 위의 노름(Norm)이란 모든 벡터 $\mathbf{x} \in V$ 에 대해 그 길이 $\|\mathbf{x}\| \in \mathbb{R}$ 를 할당하는 함수 $\|\cdot\| : V \to \mathbb{R}$ 이며, 모든 스칼라 $\lambda \in \mathbb{R}$ 및 벡터 $\mathbf{x}, \mathbf{y} \in V$ 에 대해 다음 3가지 공리를 반드시 만족해야 합니다:
...
$$\|\mathbf{x}\|_1 := \sum_{i=1}^n |x_i| \quad (\text{Eq 3.3})$$

- 기하학적 형태 (Figure 3.3 좌측): $\mathbb{R}^2$ 에서 $\|\mathbf{x}\|_1 = 1$ 인 단위 원(Unit Circle)을 그리면 원점이 아닌 마름모(Diamond) 형태가 됩니다.
...
$$\|\mathbf{x}\|_2 := \sqrt{\sum_{i=1}^n x_i^2} = \sqrt{\mathbf{x}^\top \mathbf{x}} \quad (\text{Eq 3.4})$$

- 기하학적 형태 (Figure 3.3 우측): $\mathbb{R}^2$ 에서 $\|\mathbf{x}\|_2 = 1$ 인 단위 원은 우리가 아는 매끄러운 동그라미(Circle) 형태입니다.
...
### 4️⃣ [4단계 실전 AI 연결고리]
- Support Vector Machine (SVM - Ch 12): 두 클래스 간의 마진(Margin)을 최대화하는 과정에서 마진의 크기가 $\frac{2}{\|\mathbf{w}\|_2}$ 로 정의되므로, 유클리드 노름을 최소화하는 QP 문제로 귀결됩니다.
- Ridge ($\ell_2$) & Lasso ($\ell_1$) 규제: 과적합(Overfitting)을 방지하기 위해 가중치 벡터의 노름 $\|\mathbf{w}\|$ 을 손실 함수에 페널티 항으로 추가합니다.
