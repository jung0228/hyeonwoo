...
중간항을 덧셈 기법으로 변환하여 곱셈 횟수를 4번에서 3번으로 감축:
...
재귀식 형태가 $T(n) = a T(n/b) + O(n^d)$ 꼴일 때 (입력 크기가 비율 $n/b$ 로 줄어들 때만 적용 가능), 재귀 트리를 일일이 그리지 않고 시간복잡도를 0초 만에 판별하는 공식입니다.
...
- 원리: 재귀식을 1단계, 2단계, 3단계 직접 대입 전개하여 $k$번째 일반항을 도출한 뒤, 기저 조건(Base Case, 예: $T(1)$)에 도달하는 $k$ 값을 대입하여 시간복잡도를 유도합니다.
...
- 비비교 정렬 (Non-comparison Sort): 계수 정렬(Counting Sort)처럼 값의 인덱스를 직접 사용하는 알고리즘은 대소 비교를 하지 않고 $\Omega(n \log n)$ 하한을 우회하여 $O(n + M)$ 선형 시간에 정렬할 수 있습니다.
...
1. 숫자로 직관 얻기: $n=10, b=2$ 일 때, 구간 $[10, 20]$ 내에 2의 거듭제곱이 존재하는가?
   ...
   - 10 이하의 거듭제곱은 8이고, 10을 넘어서는 첫 번째 거듭제곱은 16입니다.
...
[해설 및 전수 계산]
- $n=4$ 개짜리 수열의 FFT 고정 대입점: $\omega^0 = 1, \omega^1 = -i, \omega^2 = -1, \omega^3 = i$
- (a) 수열 $(1, 0, 0, 0)$: $A(x) = 1$ (상수 다항식) ➔ $A(1)=1, A(-i)=1, A(-1)=1, A(i)=1 \implies \mathbf{(1, 1, 1, 1)}$
- (b) 수열 $(1, 0, 1, -1)$: $f(x) = 1 + x^2 - x^3$
  - $f(1) = 1 + 1 - 1 = 1$
  - $f(-i) = 1 + (-1) - (i) = -i$
  - $f(-1) = 1 + 1 - (-1) = 3$
  - $f(i) = 1 + (-1) - (-i) = i$
...
### 📌 Exercise 2.9 (FFT 다항식 곱셈 3단계 & 역-FFT $jk$ 인덱스 원리)
[원문]
(a) Multiply $P(x) = x + 1$ and $Q(x) = x^2 + 1$ using FFT.
(b) Multiply $P(x) = 1 + x + 2x^2$ and $Q(x) = 2 + 3x$ using FFT.

[역-FFT (Inverse FFT) 개념 및 $jk$ 지수]
- 역-FFT 공식: $a_j = \frac{1}{n} \sum_{k=0}^{n-1} y_k \cdot \omega^{-jk}$  (단, $\omega^{-jk} = (+i)^{j \times k}$)
- $jk = j \times k$ 인덱스 곱셈표 ($n=4$):
  - $j$ (차수 위치 $0\dots3$), $k$ (점 위치 $0\dots3$)의 개별 곱 숫자로 지수가 결정됩니다.

[전수 풀이]
- (a) $P(x) = x + 1$, $Q(x) = x^2 + 1$ 곱셈:
  1. 4개 점 대입 (FFT): $P \to (2, 1-i, 0, 1+i)$, $Q \to (2, 0, 2, 0)$
  2. 성분별 곱셈: $(2 \times 2, (1-i) \times 0, 0 \times 2, (1+i) \times 0) = (4, 0, 0, 0)$
  3. 역-FFT 다항식 복원: $a_j = \frac{1}{4} (4 \cdot (+i)^{0}) = 1 \implies \mathbf{x^3 + x^2 + x + 1}$
- (b) $P(x) = 1 + x + 2x^2$, $Q(x) = 2 + 3x$ 곱셈:
  - 전개 및 동류항 정리: $(1 + x + 2x^2)(2 + 3x) = \mathbf{6x^3 + 7x^2 + 5x + 2}$

---

### 📌 Exercise 2.10 (다항식 보간법 - 스위치 다항식 조립 원리 & 풀이)
[원문]
...
- $y=0$ 인 $x=3, 5$ 항을 제외하고 $S_1(x), S_2(x), S_4(x)$ 3개 스위치 다항식 합산:
  ...
  - $S_4(x) = 4 \cdot \frac{(x-1)(x-2)(x-3)(x-5)}{(4-1)(4-2)(4-3)(4-5)} = -\frac{2}{3} (x^4 - 11x^3 + 41x^2 - 61x + 30)$
- 계수 합산 정리:
