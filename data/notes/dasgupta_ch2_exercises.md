...
- **수학적 아이디어**:
  두 $n$자리 수 $X, Y$를 $X = X_L \cdot 10^{n/2} + X_R$, $Y = Y_L \cdot 10^{n/2} + Y_R$ 로 분할하면:
  $$X \times Y = (X_L Y_L) \cdot 10^n + (X_L Y_R + X_R Y_L) \cdot 10^{n/2} + (X_R Y_R)$$
  이때 중간항을 덧셈 기법으로 변환하여 곱셈 횟수를 4번에서 **3번**으로 줄입니다:
  $$X_L Y_R + X_R Y_L = (X_L + X_R)(Y_L + Y_R) - X_L Y_L - X_R Y_R$$
- **시간복잡도 비교**:
  - 초등 일반 곱셈: $T(n) = 4T(n/2) + O(n) \implies \mathbf{O(n^2)}$
  - 분할 정수 곱셈 (카라츠바): $T(n) = 3T(n/2) + O(n) \implies \mathbf{O(n^{\log_2 3}) \approx O(n^{1.585})}$
