# 📐 Dasgupta Algorithm Chapter 2: Divide-and-Conquer (분할 정복 연습문제 2.1 ~ 2.32 전수 해설)

> POSTECH 대학원 지정 교재 《Algorithms (by Sanjoy Dasgupta, Christos Papadimitriou, Umesh Vazirani)》 Chapter 2 연습문제 32문항 100% 전수 해설 노트

---

## 🌐 1. [1단계 명확한 개념 정의]: 분할 정복(Divide-and-Conquer)의 수학적 정수

분할 정복(Divide-and-Conquer) 알고리즘 체계는 크게 3단계 메커니즘으로 동작합니다:
1. Divide (분할): 크기가 $n$인 문제를 동일한 유형의 더 작은 부분 문제(Subproblems) 여러 개로 분할합니다.
2. Conquer (정복): 부분 문제들을 재귀적으로(Recursively) 해결합니다. 문제 크기가 충분히 작아지면 기저 사례(Base Case)로 직접 풉니다.
3. Combine (병합): 부분 문제들의 해를 결합하여 원래 문제의 최종 해를 도출합니다.

### 💡 마스터 정리 (Master Theorem)
재귀식 $T(n) = a T(n/b) + O(n^d)$ (단, $a \ge 1, b > 1, d \ge 0$) 의 점상복잡도는 다음과 같이 확정됩니다:
...
[해설 및 증명]
1. 배열 $A$를 MergeSort로 정렬합니다 ➔ $O(n \log n)$ 소요.
