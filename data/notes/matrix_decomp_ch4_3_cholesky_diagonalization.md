...
숄레스키 분해 $A = L L^\top$ 는 "양변을 직접 곱한 후, 같은 자리의 원소끼리 1대1로 비교하여 미지수 $l_{ij}$ 를 하나씩 건져 올리는 미정계수법"으로 도출됩니다:

1. 하삼각행렬 $L$ 과 $L^\top$ 의 직접 곱셈 전개:
   ...
   $$L L^\top = \begin{bmatrix} l_{11}^2 & l_{21}l_{11} & l_{31}l_{11} \\\\ l_{21}l_{11} & l_{21}^2 + l_{22}^2 & l_{31}l_{21} + l_{32}l_{22} \\\\ l_{31}l_{11} & l_{31}l_{21} + l_{32}l_{22} & l_{31}^2 + l_{32}^2 + l_{33}^2 \end{bmatrix}$$

2. 3단계 열별(Column-by-Column) 1대1 위치 비교 및 미지수 도출:
   - Step 1 (첫 번째 열 원소 정복):
     - 1행 1열: $a_{11} = l_{11}^2 \implies l_{11} = \sqrt{a_{11}}$
     - 2행 1열: $a_{21} = l_{21}l_{11} \implies l_{21} = \frac{a_{21}}{l_{11}}$
     - 3행 1열: $a_{31} = l_{31}l_{11} \implies l_{31} = \frac{a_{31}}{l_{11}}$
   - Step 2 (두 번째 열 원소 정복):
     - 2행 2열: $a_{22} = l_{21}^2 + l_{22}^2 \implies l_{22} = \sqrt{a_{22} - l_{21}^2}$
     - 3행 2열: $a_{32} = l_{31}l_{21} + l_{32}l_{22} \implies l_{32} = \frac{a_{32} - l_{31}l_{21}}{l_{22}}$
   - Step 3 (세 번째 열 원소 정복):
     - 3행 3열: $a_{33} = l_{31}^2 + l_{32}^2 + l_{33}^2 \implies l_{33} = \sqrt{a_{33} - (l_{31}^2 + l_{32}^2)}$

3. 일반 $n \times n$ 차원 숄레스키 점화식:
   - 대각 성분 ($l_{ii}$): $l_{ii} = \sqrt{a_{ii} - \sum_{k=1}^{i-1} l_{ik}^2} \quad (\text{Eq 4.47})$
   - 비대각 성분 ($l_{ij}, \; i > j$): $l_{ij} = \frac{1}{l_{jj}} \left( a_{ij} - \sum_{k=1}^{j-1} l_{ik}l_{jk} \right) \quad (\text{Eq 4.48})$

- 계산 복잡도가 절반($\frac{1}{3}n^3$)으로 줄어드는 이유:
  일반적인 LU 분해는 위삼각 $U$ 와 아래삼각 $L$ 의 $n^2$ 개 성분을 모두 따로 구해야 하므로 $\frac{2}{3}n^3$ 연산이 듭니다.
  하지만 숄레스키 분해는 $A$ 가 대칭 행렬이므로 하삼각행렬 $L$ 의 $\frac{n(n+1)}{2}$ 개 성분만 구하면 윗부분 $L^\top$ 은 전치 복사로 공짜로 얻어집니다.
  따라서 계산량이 정확히 절반인 $\frac{1}{3}n^3$ 으로 줄어들며, 부동소수점 피벗팅 없이도 수치적으로 완벽한 안정성을 가집니다.
...
- 숄레스키($A=LL^\top$) vs 고유값 분해($A=PDP^\top$) 계산 복잡도 및 적용 비교:
  - 숄레스키 분해 ($O(n^3/3)$): 
    대칭 양의 정정(SPD) 행렬에만 사용 가능하지만, 하삼각행렬 $L$ 만 구하면 윗부분 $L^\top$ 은 전치로 공짜로 얻어지므로 계산량이 일반 행렬 분해의 절반인 $O(n^3/3)$ 으로 초고속이며 수치적으로 극도로 안정적입니다.
  - 고유값 분해 ($O(n^3)$): 
    비결함 정방행렬의 모든 고유축과 고유스펙트럼을 분리해 냅니다.
    $n$차원 공간에서 $n$개의 고유벡터 축을 찾아 정렬할 때, 축 1개를 회전 정렬하는 데 $n^2$ 번의 연산이 들고 이를 $n$개 축에 대해 반복하므로 총 연산량은 $n \times n^2 = O(n^3)$ 이 소요됩니다.
    비록 연산량은 숄레스키보다 더 들지만 공간의 모든 불변 주축을 완벽하게 분석할 수 있습니다.
