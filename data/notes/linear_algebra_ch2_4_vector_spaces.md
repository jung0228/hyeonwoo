...
> 💡 핵심 관점: 군(Group)은 단순히 집합만 뜻하는 것이 아니라, [집합 $G$ + 연산 $\otimes$] 세트 순서쌍 $(G, \otimes)$ 형태로 묶어서 함께 다룹니다!
...
- $(\mathbb{Z}, +)$ 정수 덧셈: 덧셈 닫힘, 항등원 0, 역원 $-z$ 가 모두 존재하므로 아벨군입니다.
- $(\mathbb{N}_0, +)$ 자연수와 0: 항등원 0은 있지만, 양수 $5$를 $0$으로 만들 음수 역원(-5)이 집합에 없어서 역원 파탄 (군 아님).
- $(\mathbb{Z}, \cdot)$ 정수 곱셈: 항등원 1은 있지만, $3 \cdot x = 1$ 을 만들 역원($1/3$)이 정수가 아니라서 역원 파탄 (군 아님).
- $(\mathbb{R}, \cdot)$ 실수 곱셈: 항등원 1은 있지만, 숫자 0은 곱해서 1을 만들 수 있는 역원($1/0$)이 존재하지 않아 군 파탄!
