...
- 주의 (Remark): 모든 내적은 노름을 유도하지만, 모든 노름이 내적으로부터 유도되는 것은 아닙니다!
  - 예: 맨해튼 노름($\ell_1$ norm: $\|\mathbf{x}\|_1 = \sum |x_i|$)은 대응하는 내적이 존재하지 않는 대표적인 노름입니다.
...
벡터 $\mathbf{x} = \begin{bmatrix} 1 \\ 1 \end{bmatrix}$ 의 길이는 어떤 내적을 채택하느냐에 따라 물리적 길이가 달라집니다!
...
2. 가중치 행렬 $A = \begin{bmatrix} 1 & -1/2 \\ -1/2 & 1 \end{bmatrix}$ 내적 적용 시:
   ...
   - 동일한 벡터라도 이 내적 공간에서는 도트 곱 공간보다 더 "짧게" 측정됩니다.

### 📌 4. 거리(Distance)와 거리함수(Metric: Definition 3.6 & Eq 3.21~3.23)
내적 공간 $(V, \langle \cdot, \cdot \rangle)$ 상의 두 벡터 $\mathbf{x}, \mathbf{y}$ 사이의 거리 $d(\mathbf{x}, \mathbf{y})$ 는 다음과 같이 유도된 노름으로 정의됩니다:
...
- 내적 vs 거리의 반대 동작 성질 (Remark):
  두 벡터가 서로 매우 유사할수록 내적 $\langle \mathbf{x}, \mathbf{y} \rangle$ 은 큰 값을 갖고, 거리 $d(\mathbf{x}, \mathbf{y})$ 는 0에 가까운 작은 값을 가집니다!
...
코시-슈바르츠 부등식에 의해 $-1 \le \frac{\langle \mathbf{x}, \mathbf{y} \rangle}{\|\mathbf{x}\| \|\mathbf{y}\|} \le 1$ 이 항상 성립하므로, 두 0이 아닌 벡터 $\mathbf{x}, \mathbf{y}$ 사이의 각도 $\omega \in [0, \pi]$ 는 다음과 같이 유일하게 정의됩니다:
...
- 직교 (Orthogonal): 두 벡터의 내적값이 0일 때 두 벡터는 직교한다고 하며 $\mathbf{x} \perp \mathbf{y}$ 로 표기합니다:
  $$\mathbf{x} \perp \mathbf{y} \iff \langle \mathbf{x}, \mathbf{y} \rangle = 0$$
- 정규직교 (Orthonormal): 두 벡터가 직교하면서 동시에 각각의 노름(길이)이 1인 단위 벡터일 때 정규직교라 부릅니다 ($\mathbf{x} \perp \mathbf{y}$ 이고 $\|\mathbf{x}\| = 1 = \|\mathbf{y}\|$).
- 영벡터의 성질: 영벡터 $\mathbf{0}$ 은 공간 상의 모든 벡터와 직교합니다.

### 📌 3. 내적에 따른 직교성의 변화 (Example 3.7 & Eq 3.27~3.28)
$\mathbf{x} = [1, 1]^\top, \mathbf{y} = [-1, 1]^\top$ 두 벡터는:
1. 표준 도트 곱 기준: $\mathbf{x}^\top \mathbf{y} = -1 + 1 = 0 \implies 90^\circ$ 로 직교함!
2. 가중치 내적 $\langle \mathbf{x}, \mathbf{y} \rangle = \mathbf{x}^\top \begin{bmatrix} 2 & 0 \\ 0 & 1 \end{bmatrix} \mathbf{y}$ 기준:
   ...
   즉, 어떤 내적 기준에서는 직교하는 벡터가 다른 내적 기준에서는 직교하지 않을 수 있습니다!

### 📌 4. 직교 행렬 (Orthogonal Matrix: Definition 3.8 & Eq 3.29~3.30)
정방행렬 $A \in \mathbb{R}^{n \times n}$ 의 열벡터들이 서로 정규직교(Orthonormal)집합을 이룰 때 $A$ 를 직교 행렬(Orthogonal Matrix)이라 부릅니다:

$$A A^\top = I = A^\top A \implies A^{-1} = A^\top \quad (\text{Eq 3.29~3.30})$$

- 용어 주의: 관례상 "Orthogonal Matrix"라 부르지만, 실제로는 열벡터들이 단지 직교할 뿐만 아니라 길이가 1인 "Orthonormal Matrix (정규직교 행렬)"을 의미합니다.

### 📌 5. 직교 행렬 변환의 2대 불변 성질 (Length & Angle Preservation: Eq 3.31~3.32)
직교 행렬 $A$ 로 벡터를 변환(회전 또는 반사)하더라도 길이(Distance)와 각도(Angle)가 100% 보존됩니다!
